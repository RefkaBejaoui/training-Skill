...
//         if (!studentResponse) {
//             return res.status(404).send({msg:"studentresponse not found"});
//         }
//             let studentScore =0;
            
//     studentResponse.checkPointQuestions.forEach((checkPointQuestion, index) => {
//         const selectedResponses = studentResponse.responses[index];
  
//         const allCorrect = checkPointQuestion.correctAnswer.every(answer => selectedResponses.includes(answer));
//         const tooManySelected = selectedResponses.length > checkPointQuestion.correctAnswer.length;
...
//            studentResponsesId : studentResponses._id
//         }));
...
router.post("/registerScore", async (req, res) => {
  try {
    ...
    // Retrieve student's stored responses from the database
    const studentResponse = await Response.findOne({ studentId: studentId });
    ...
    // Iterate through each response and calculate score
    studentResponse.checkPointQuestions.forEach((checkPointQuestion, index) => {
      const selectedResponses = studentResponse.responses[index];
